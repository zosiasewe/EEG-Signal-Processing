function clean_eeg = ICA_subject(EEG_data, n_components, sampling_rate, subject_name, class, epoch_time)
    [n_channels, n_samples, n_trials] = size(EEG_data);
    clean_eeg = zeros(n_channels, n_samples, n_trials);

    % Reshape to [channels x (samples*trials)]
    EEG_2d = reshape(EEG_data, n_channels, []);

    [ica_components, W_matrix, A_matrix] = ICA(EEG_2d, n_components, 500, 1e-6, 1);

    % Reshape to [components x samples x trials]
    ica_reshaped = reshape(ica_components, n_components, n_samples, n_trials);

    trial_for_ica = 1;
    ICA_plot(ica_reshaped, sampling_rate, epoch_time, subject_name, class, trial_for_ica);
