function fuzzy_features = applyChromosome(chromosome, feature_matrix_normalized, n_extracted_features, n_fuzzy_terms)
    ...
    for feat_idx = 1:n_extracted_features
        ...
        w = comb_weights(feat_idx,:);
        extracted_features(:,feat_idx) = w(1)*x + w(2)*exp_comp + w(3)*sin_comp + w(4)*log_comp + w(5)*pow_comp;
    end
    ...
    for feat_idx = 1:n_extracted_features
        ...
        for term = 1:n_fuzzy_terms
            a = fuzzy_params(feat_idx, term, 1);
            b = fuzzy_params(feat_idx, term, 2);
            c = fuzzy_params(feat_idx, term, 3);
