function fuzzy_features = applyChromosome(chromosome, feature_matrix_normalized, n_extracted_features, n_fuzzy_terms)
    ...
    % takes a chromosome and applies it to your actual data to create new features
    % Get dimensions
    
    n_trials = size(feature_matrix_normalized, 1);
    
    % Extract chromosome components
    linear_weights = chromosome.linear_weights;
    nonlinear_exp = chromosome.nonlinear_exp;
    nonlinear_sin = chromosome.nonlinear_sin;
    nonlinear_log = chromosome.nonlinear_log;
    nonlinear_pow = chromosome.nonlinear_pow;
    comb_weights = chromosome.comb_weights;
    fuzzy_params = chromosome.fuzzy_params;
    ...
    for feat_idx = 1:n_extracted_features
        ...
        % Numerical stability (prevent overflow in nonlinear functions)
        x = max(-10, min(10, x));
    ...
    for feat_idx = 1:n_extracted_features
        ...
        for term = 1:n_fuzzy_terms
            ...
            % Triangular membership function with  stability
