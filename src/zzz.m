...
% 6. Test ratio comparison - NEW CONFIGURATION
param_sets{end+1} = struct(...
    ...
    'mu_list', [20], ...
    'lambda_list', [60], ...
    'test_ratio_list', [0.1, 0.15, 0.2, 0.25, 0.3]);

fixedParams = struct();
fixedParams.n_fuzzy_terms = 3;
fixedParams.T_max = 150;
fixedParams.k_folds_cv = 5;
% NOTE: test_ratio is now variable per configuration, not fixed
            ...
            for k_selected_features = k_selected_features_list
                for lambda_numbers = lambda_list
                    for mu_numbers = mu_list
                        for test_ratio = test_ratio_list  % NEW LOOP
                            ...
                            for run = 1:N_RUNS
                                ...
                                rng('shuffle'); % FIXED: proper random seeding
                               
                                % train/test split - NOW USING VARIABLE TEST_RATIO
